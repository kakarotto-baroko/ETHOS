...
import json, os, math, hashlib, datetime, argparse
...
# ✅ 支持命令行参数 + 环境变量双通道（兼容 GitHub Actions）
parser = argparse.ArgumentParser()
parser.add_argument("--cadence", default=None, help="Cadence type: daily / t+3 / weekly")
args = parser.parse_args()

CADENCE = (args.cadence or os.getenv("CADENCE", "daily")).strip().lower()
...
def pct01(x): return max(0.0, min(1.0, float(x)))
def round1(x): return float(f"{x:.1f}")

def cadence_tag(cadence: str) -> str:
    return {"daily": "daily", "tplus3": "t+3", "t+3": "t+3", "weekly": "weekly"}.get(cadence, "daily")

def cadence_boost(keybase: str, base: float) -> float:
    """根据 cadence 对强度做微调"""
    r = stable_rand_0_1(f"{keybase}|{CADENCE}")
    if CADENCE == "weekly":
        return pct01(base * (0.85 + 0.40 * r))
    if CADENCE in ("tplus3", "t+3"):
        return pct01(base * (0.90 + 0.20 * r))
    return pct01(base * (0.95 + 0.10 * r))
...
def gen_board():
    ...
    for (bucket, structure, stage, name) in SEED_PROJECTS:
        ...
        score = (
            0.18*agents["A1_vol_mcap"] + 0.15*agents["A2_tvl"] +
            0.12*agents["A3_sns"] + 0.10*agents["A4_depth"] +
            0.15*agents["B1_leverage"] + 0.10*agents["B2_whale"] +
            0.10*agents["B3_dev"] + 0.10*agents["B4_ecosys"]
        ) / 100.0

        score_total = round1(score*100)
        tvl3 = round1(-2.0 + 8.0 * stable_rand_0_1(key+":tvl3"))
        vol1 = round1(2.0 + 8.0 * stable_rand_0_1(key+":vol1"))
...
def gen_sensors():
    ...
    for layer, probes in SENSOR_LAYERS.items():
        ...
        q = round1(pct01(cadence_boost(layer, q_base)) * 100)
        pr = []
        for k in probes:
            key = f"{layer}|{k}|{CADENCE}"
            if k in ("lp_lock_ratio","holder_gini"):
                v = round1(50 + 50*stable_rand_0_1(key))
            elif k in ("tg_growth_7d","x_engagement","news_pulse"):
                v = round1(-2.0 + 8.0*stable_rand_0_1(key))
            elif k=="depth_usd":
                v = int(50_000 + 200_000*stable_rand_0_1(key))
            elif k=="commit_7d":
                v = int(5 + 40*stable_rand_0_1(key))
